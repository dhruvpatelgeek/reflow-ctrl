...
MY_MOSI   EQU  P2.6 ;P2.2  ;MOSI
MY_MISO   EQU  P2.5   ;MISO
MY_SCLK   EQU  P2.4   ;SPICLK
    ...
    INC_DEC_Reflow_time:
        ...
        PushButton(SETUP_SOAK_Button,check_decrement) ; setup soak is also used to increment 
          ...
          mov a, reflow_time+1
            ...
            mov reflow_time+1, a
            mov a, reflow_time+1
            jnz INC_reflow_time_done2
            mov a, reflow_time+0
            add a, #0x01
            da a ; Decimal adjust instruction.  Check datasheet for more details!
            mov reflow_time+0, a
           ; mov a, reflow_temp+1
            INC_reflow_time_done2:

        check_decrement:
        
            PushButton( Button_min, INC_reflow_time_done)
            mov a, reflow_time+1
            add a, #0x99
            da a ; Decimal adjust instruction.  Check datasheet for more details!
            mov reflow_time+1, a
            mov a, reflow_time+1
            jnz INC_reflow_time_done
            mov a, reflow_time+0
            add a, #0x99
            da a ; Decimal adjust instruction.  Check datasheet for more details!
            mov reflow_time+0, a
          ;  mov a, reflow_temp+1
            INC_reflow_time_done:
        ret
    INC_DEC_Reflow_temp:
...
;	Set_Cursor(1, 1)
  ;  Display_BCD(BCD_counter)
    ...
    clr tt_reflow_flag
    mov soak_time, #0x00
    mov soak_time+1, #0x50
    mov reflow_time, #0x00
    mov reflow_time+1, #0x50
    mov soak_temp, #0x01
    mov reflow_temp+0, #0x01
    mov reflow_temp+1, #0x50
    clr tt_reflow_flag
